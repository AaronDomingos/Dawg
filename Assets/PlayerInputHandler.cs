...
public class PlayerInputHandler : InputHandler
{
    ...
    public void Handle()
    {
        ...
        if (userInput.User.Toggle.WasReleasedThisFrame() && player.CanToggle)
        {
        ...
        if (userInput.User.Index1.WasReleasedThisFrame() && 
            player.CanToggle) {player.GoToPlayableIndex(0);}
        if (userInput.User.Index2.WasReleasedThisFrame() && 
            player.CanToggle) {player.GoToPlayableIndex(1);}
        if (userInput.User.Index3.WasReleasedThisFrame() && 
            player.CanToggle) {player.GoToPlayableIndex(2);}
        if (userInput.User.Index4.WasReleasedThisFrame() && 
            player.CanToggle) {player.GoToPlayableIndex(3);}
        if (userInput.User.Index5.WasReleasedThisFrame() && 
            player.CanToggle) {player.GoToPlayableIndex(4);}
        if (userInput.User.Index6.WasReleasedThisFrame() && 
            player.CanToggle) {player.GoToPlayableIndex(5);}
        if (userInput.User.Index7.WasReleasedThisFrame() && 
            player.CanToggle) {player.GoToPlayableIndex(6);}
        if (userInput.User.Index8.WasReleasedThisFrame() && 
            player.CanToggle) {player.GoToPlayableIndex(7);}
        if (userInput.User.Index9.WasReleasedThisFrame() && 
            player.CanToggle) {player.GoToPlayableIndex(8);}
        if (userInput.User.Index0.WasReleasedThisFrame() && 
            player.CanToggle) {player.GoToPlayableIndex(9);}
        #endregion
    }
}
